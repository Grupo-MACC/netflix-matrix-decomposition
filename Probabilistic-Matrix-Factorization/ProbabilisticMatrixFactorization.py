...
class PMF(object):
    ...
    def fit(self, train_vec, test_vec):
        # mean subtraction
        self.mean_inv = np.mean(train_vec[:, 2])  # Calcula el promedio global de las valoraciones

        pairs_train = train_vec.shape[0]    # nº de ratings en train
        pairs_test = test_vec.shape[0]      # nº de ratings en test

        # 1-p-i, 2-m-c. Calcula número total de usuarios e items (máx id + 1).
        ...
        while self.epoch < self.maxepoch:  # Entrenamiento por épocas
            self.epoch += 1
            ...
            for batch in range(self.num_batches):  # 每次迭代要使用的数据量
                ...
                # Compute Objective Function Rui = Uu*Vi
                pred_out = np.sum(np.multiply(self.w_User[batch_UserID, :],
                                              self.w_Item[batch_ItemID, :]),
                                  axis=1)  # mean_inv subtracted # np.multiply对应位置元素相乘

                rawErr = pred_out - train_vec[shuffled_order[batch_idx], 2] + self.mean_inv # error = predicción – rating real + media global
