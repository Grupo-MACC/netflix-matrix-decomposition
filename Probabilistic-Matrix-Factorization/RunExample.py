...
if __name__ == "__main__":
    ...
    '''
    Imprime 943 1682 10:
    nº de usuarios distintos (ratings[:,0]) → 943,
    nº de películas distintas (ratings[:,1]) → 1682,
    nº de características latentes (pmf.num_feat) → 10.
    '''
    train, test = train_test_split(ratings, test_size=0.2)  # spilt_rating_dat(ratings)
    pmf.fit(train, test)  # Entrena el modelo PMF con los datos de train y evalúa sobre test.
